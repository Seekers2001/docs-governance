...
3. **要改接口 → 提契约变更请求。** 不在实现里偷改。模式 A 回报 director；模式 B 写进约定位置（契约"待定变更"区或 `PROJECT_LOG.md` 的 `contract-request`），由契约拥有者裁决后更新契约。破坏性变化必须同时写明兼容期、消费者迁移顺序、回滚条件和不可逆部分；缺失时不进入实现。
